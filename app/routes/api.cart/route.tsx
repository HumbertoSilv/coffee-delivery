...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  if (action === "add") {
    ...
    if(!alreadyExists) {
      ...
      return json({ cart }, { headers: { "Set-Cookie": await cartSession.commitSession(session) } })
    }
    ...
    return json({ updatedCart }, { headers: { "Set-Cookie": await cartSession.commitSession(session) } })
  }

  if (action === "sub") {
    const item = cart.find(({ product: item }) => item.id === product.id)

    if(!item || item.quantity < 2) return json({ headers: { "Set-Cookie": await cartSession.commitSession(session) }})
